...
const Navbar = () => {
  ...
  const categories = {
    ...
    "bolt-tensioner": [
        ...
        { id: "spring-return-tensioner", name: "Spring Return Bolt Tensioner", img: "/product5.png", path: "/springreturnbolt", width: 350, height: 400 },
      ...
      <div className="">
        <div className="flex items-center h-12 lg:gap-6 xl:gap-16 2xl:gap-20">
          ...
          <div className="hidden md:flex flex-wrap items-center md:gap-x-3 lg:gap-x-6 xl:gap-x-8 2xl:gap-x-10 gap-y-2">
            <Link
              href="/aboutus"
              className={`text-white font-bold text-base transition-colors duration-200 hover:border-b-2 hover:border-red-500 ${pathname === "/aboutus" ? "border-b-2 border-red-500" : ""}`}
            >
              About us
            </Link>
            ...
            <div className="relative">
              <div className="flex items-center">
                <a
                  href="/product"
                  className={`text-white font-bold text-base transition-colors duration-200 hover:border-b-2 hover:border-red-500 ${pathname === "/product" ? "border-b-2 border-red-500" : ""}`}
                >
                  Products
                </a>
                <button
              ...
              {isProductsOpen && (
                ...
                >
                  {/* Main Categories */}
                  <div className="flex font-bold flex-col h-full w-[300px]">
                    <button onClick={() => { setActiveSubmenu("insitu-machinning"); setActiveChildMenu([]); setSelectedProduct(null);}} className={`flex items-center w-full flex-1 px-4 text-base text-gray-800 text-left ${activeSubmenu === "insitu-machinning" ? "bg-[#D9D9D9]" : "hover:bg-[#D9D9D9]"}`}><FaAngleDoubleRight className="text-red-500 text-xl flex-shrink-0 md:mr-4" />Insitu Machinning</button>
                    <button onClick={() => { setActiveSubmenu("bolting-tools"); setActiveChildMenu([]); setSelectedProduct(null);}} className={`flex items-center w-full flex-1 px-4 text-base text-gray-800 text-left ${activeSubmenu === "bolting-tools" ? "bg-[#D9D9D9]" : "hover:bg-[#D9D9D9]"}`}><FaAngleDoubleRight className="text-red-500 text-xl flex-shrink-0 md:mr-4" />Bolting Tools</button>
                    <button onClick={() => { setActiveSubmenu("hydraulic-powerpack"); setActiveChildMenu([]); setSelectedProduct(null);}} className={`flex items-center w-full flex-1 px-4 text-base text-gray-800 text-left ${activeSubmenu === "hydraulic-powerpack" ? "bg-[#D9D9D9]" : "hover:bg-[#D9D9D9]"}`}><FaAngleDoubleRight className="text-red-500 text-xl flex-shrink-0 md:mr-4" />Hydraulic Torquing Power Pack</button>
                    <button onClick={() => { setActiveSubmenu("accesories"); setActiveChildMenu([]); setSelectedProduct(null);}} className={`flex items-center w-full flex-1 px-4 text-base text-gray-800 text-left ${activeSubmenu === "accesories" ? "bg-[#D9D9D9]" : "hover:bg-[#D9D9D9]"}`}><FaAngleDoubleRight className="text-red-500 text-xl flex-shrink-0 md:mr-4" />Accesories</button>
                    <button onClick={() => { setActiveSubmenu("impact-sockets"); setActiveChildMenu([]); setSelectedProduct(null);}} className={`flex items-center w-full flex-1 px-4 text-base text-gray-800 text-left ${activeSubmenu === "impact-sockets" ? "bg-[#D9D9D9] rounded-bl-2xl" : "hover:bg-[#D9D9D9]"}`}><FaAngleDoubleRight className="text-red-500 text-xl flex-shrink-0 md:mr-4" />Sockets & Reducers</button>
                  </div>

                  {/* Subcategories */}
                  {activeSubmenu && (
                    <div className="w-[350px] text-gray-800 h-full font-bold flex flex-col border-l border-gray-200 bg-gray-50 overflow-y-auto">
                      {categories[activeSubmenu].map((item) => (
                        <div key={item.id} className="flex flex-col">
                          <button
                            onClick={() => {
                            ...
                            // Added text-base and py-4
                            className={`flex items-center gap-2 text-left py-4 px-4 hover:bg-gray-100 text-base ${ selectedProduct?.id === item.id ? "text-red-500 font-semibold" : "" } ${ activeChildMenu.includes(item.id) ? "text-red-600" : "" }`}
                          >
                          ...
                          {/* Child Submenu */}
                          {activeChildMenu.includes(item.id) && categories[item.id] && (
                            // Added py-2 for vertical padding around the child list
                            <div className="ml-8 flex flex-col text-gray-500 font-semibold gap-1 py-2">
                              {categories[item.id].map((child) => (
                                <button
                                  key={child.id}
                                  onClick={() => setSelectedProduct(child)}
                                  // Added text-base and py-3
                                  className={`text-left py-3 px-3 text-base ${ selectedProduct?.id === child.id ? "text-red-500 font-bold" : "hover:text-red-500" }`}
                                >
                                  {child.name}
                                </button>
                              ))}
                  ...
                  {/* Image Preview */}
                  {selectedProduct && (
                    <div className="w-[450px] flex items-center justify-center h-full border-l border-gray-200 rounded-br-2xl bg-white p-4">
                      <div className="text-center">
                        {selectedProduct.img ? (
                          <img
                            src={selectedProduct.img.startsWith('/') ? selectedProduct.img : `/${selectedProduct.img}`}
                            alt={selectedProduct.name}
                            className="object-contain rounded-md cursor-pointer hover:opacity-90 transition max-h-[400px]"
                            style={{ width: `${selectedProduct.width || 400}px`, height: selectedProduct.height ? `${selectedProduct.height}px` : 'auto' }}
                            onClick={() => { closeProductDropdown(); router.push(selectedProduct.path.startsWith('/') ? selectedProduct.path : `/${selectedProduct.path}`); }}
                          />
                        ) : (
                          <div
                            className="flex items-center justify-center text-gray-500 cursor-pointer border border-gray-300 rounded-md bg-gray-100"
                            style={{ width: `${selectedProduct.width || 400}px`, height: `${selectedProduct.height || 300}px` }}
                            onClick={() => { closeProductDropdown(); router.push(selectedProduct.path.startsWith('/') ? selectedProduct.path : `/${selectedProduct.path}`); }}
                          >
                            Image not provided
                          </div>
                        ...
                        <div
                          className="text-red-500 font-bold text-base text-center capitalize mt-4 cursor-pointer hover:text-red-700"
                          onClick={() => { closeProductDropdown(); router.push(selectedProduct.path.startsWith('/') ? selectedProduct.path : `/${selectedProduct.path}`); }}
                        >
                          {selectedProduct.name}
                        </div>
            ...
            <div className="relative">
              ...
              {isServicesOpen && (
                <div
                  ref={serviceRef}
                  className={`absolute top-full mt-6 bg-white border border-gray-200 shadow-lg z-100 h-[500px] rounded-bl-2xl rounded-br-2xl flex ml-[-300] transition-all duration-300 ${ selectedService ? 'w-[700px]' : 'w-[300px]' }`}
                >
                  {/* Service list */}
                  <div className="flex flex-col h-full w-[300px] font-bold overflow-y-auto">
                    {servicesList.map((svc, index) => (
                      <button
                        key={svc.id}
                        onClick={() => setSelectedService(svc)}
                        // Added text-base and py-4
                        className={`flex items-center w-full flex-shrink-0 py-4 px-4 text-base text-gray-800 text-left ${ selectedService?.id === svc.id ? "bg-[#D9D9D9]" : "hover:bg-[#D9D9D9]" } ${ index === servicesList.length - 1 ? 'rounded-bl-2xl' : '' }`}
                      >
                        <BsDash className="w-7 text-red-500 h-10 flex-shrink-0 mr-2" /> {svc.name}
                      </button>
                    ))}
                  </div>
                  ...
                  {/* Image preview */}
                  {selectedService && (
                    <div className="w-[400px] flex flex-col items-center justify-center h-full border-l border-gray-200 bg-white rounded-br-2xl p-4">
                      <div className="flex-1 flex items-center justify-center">
                        {selectedService.img ? (
                          <img
                            src={selectedService.img.startsWith('/') ? selectedService.img : `/${selectedService.img}`}
                            alt={selectedService.name}
                            className="object-contain cursor-pointer hover:opacity-90 transition rounded-md max-h-[350px]"
                            onClick={() => { closeServiceDropdown(); router.push(selectedService.path.startsWith('/') ? selectedService.path : `/${selectedService.path}`); }}
                          />
                        ) : (
                          <div
                            className="flex items-center justify-center text-gray-500 cursor-pointer border border-gray-300 rounded-md bg-gray-100 h-[300px] w-[300px]"
                            onClick={() => { closeServiceDropdown(); router.push(selectedService.path.startsWith('/') ? selectedService.path : `/${selectedService.path}`); }}
                          >
                            Image not provided
                          </div>
                      ...
                      <div className="h-16 flex items-center justify-center mt-4">
                        <div
                          className="text-red-500 font-bold text-lg text-center capitalize cursor-pointer hover:text-red-700 transition"
                           onClick={() => { closeServiceDropdown(); router.push(selectedService.path.startsWith('/') ? selectedService.path : `/${selectedService.path}`); }}
                        >
                          {selectedService.name}
                        </div>
            ...
            <Link href="/distributor" className={`text-white hover:border-b-2 border-red-500 transition-colors duration-200 text-base font-bold ${pathname === "/distributor" ? "border-b-2 border-red-500" : ""}`}>Distributor</Link>
            <Link href="/industry" className={`text-white hover:border-b-2 border-red-500 transition-colors duration-200 text-base font-bold ${pathname === "/industry" ? "border-b-2 border-red-500" : ""}`}>Industries</Link>
            <Link href="/contact" className={`text-white hover:border-b-2 border-red-500 transition-colors duration-200 text-base font-bold ${pathname === "/contact" ? "border-b-2 border-red-500" : ""}`}>Contact us</Link>
            <Link href="/Joinus" className={`text-white hover:border-b-2 border-red-500 transition-colors duration-200 text-base font-bold ${pathname === "/Joinus" ? "border-b-2 border-red-500" : ""}`}>Join us</Link>
            <Link href="/download" className={`hidden lg:block text-white hover:border-b-2 border-red-500 transition-colors duration-200 text-base font-bold ${pathname === "/download" ? "border-b-2 border-red-500" : ""}`}>Downloads</Link>
          </div>
        </div>
      </div>
    </nav>
